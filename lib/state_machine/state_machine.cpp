...
SensorData StateMachine::get_sensor_data()
{
	return {
		...
		_temp_sensor.is_stable(),
		_ph_sensor.is_stable()};
}
...
void StateMachine::run()
{
	...
	case State::STABLE_CHECK:
		...
		{
			_last_check = _now;
			_temp_sensor.check_stability(_temp_sensor.get_raw_value());
			_ph_sensor.check_stability(_ph_sensor.get_raw_value());
		}
...
void StateMachine::create_mqtt_payload()
{
	snprintf(_mqtt_payload, MQTT_PAYLOAD_SIZE,
			 ...
			 _temp_sensor.get_value(), _ph_sensor.get_value(),
			 _temp_sensor.get_raw_value(), _ph_sensor.get_raw_value());
}
